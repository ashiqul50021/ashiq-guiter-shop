...
const MangeAllOrders = () => {
  ...
  // get all orders
  useEffect(() => {
    fetch("https://fierce-inlet-93119.herokuapp.com/allOrders")
      .then((res) => res.json())
  ...
  const handleApprove = (id) => {
    ...
    if (proceed) {
      axios
        .put(`https://fierce-inlet-93119.herokuapp.com/approveBooking/${id}`)
        .then((result) => {
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      fetch(
        `https://fierce-inlet-93119.herokuapp.com/DeleteManageBooking/${id}`,
        {
          method: "DELETE",
